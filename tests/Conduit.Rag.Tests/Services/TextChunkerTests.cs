...
public class TextChunkerTests
{
    ...
    {
        var text    = string.Join(". ", Enumerable.Range(1, 25).Select(i => $"Sentence {i}"));
        var chunker = CreateChunker(maxSize: 100, overlap: 20);
    ...
    {
        var text    = string.Join(". ", Enumerable.Range(1, 50).Select(i => $"Sentence {i} with content"));
        var chunker = CreateChunker(maxSize: 100, overlap: 20);
        ...
        foreach (var chunk in chunks)
            Assert.That(chunk.Text.Length, Is.LessThanOrEqualTo(100),
                $"Chunk {chunk.Index} has {chunk.Text.Length} chars");
    }
    ...
    public void PrefersSentenceBoundary_OverHardCut()
    {
        var text =
            ...
            "Fourth sentence is longer and extends beyond the limit to force a split.";
    ...
    public void DefaultOptions_TextUnder2000_IsNotChunked()
    {
        ...
        Assert.That(chunker.Chunk(text), Has.Count.EqualTo(1));
    }
    ...
    public void DefaultOptions_TextOver2000_IsChunked()
    {
        ...
        Assert.That(text.Length, Is.GreaterThan(2000), "Precondition: text must exceed 2000 chars");

        var chunker = CreateChunker();

        Assert.That(chunker.Chunk(text).Count, Is.GreaterThan(1));
    }
